    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[16px] font-bold tracking-tight text-foreground">Analytics Access Control</h3>
            ...
            <tr className="border-b border-border">
              ...
              {ANALYTICS_PERMISSIONS.map((p) => (
                <th key={p.key} className="px-3 py-2.5 text-center font-semibold text-foreground">
                  {p.label}
                  <span className="mt-0.5 block text-[13px] font-normal text-muted-foreground">{p.hint}</span>
                </th>
              ))}
            </tr>
          </thead>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-secondary/40 px-3 py-2.5">
        <span className="text-[14px] font-semibold text-foreground">Preview as role</span>
        <div className="flex flex-wrap gap-1.5">
          {ANALYTICS_ROLES.map((r: AnalyticsRole) => (
            <button
              ...
              type="button"
              onClick={() => setRole(r)}
              className={`rounded-lg border px-2.5 py-1 text-[14px] font-semibold transition-colors ${
                role === r
